...
**Price:** ₹${ctx.price?.current || 'N/A'} (${ctx.price?.changePercent != null ? (ctx.price.changePercent >= 0 ? '+' : '') + ctx.price.changePercent + '%' : 'N/A'} today)
**52W Range:** ₹${ctx.price?.weekLow52 || 'N/A'} – ₹${ctx.price?.weekHigh52 || 'N/A'}
...
- SMA 20: ₹${ctx.indicators?.sma20 || 'N/A'} | SMA 50: ₹${ctx.indicators?.sma50 || 'N/A'}
...
- EPS: ₹${ctx.fundamentals?.eps != null ? ctx.fundamentals.eps.toFixed(2) : 'N/A'}
...
- Market Cap: ₹${ctx.fundamentals?.marketCapCr != null ? (ctx.fundamentals.marketCapCr >= 100000 ? (ctx.fundamentals.marketCapCr / 100000).toFixed(1) + 'L Cr' : ctx.fundamentals.marketCapCr >= 1000 ? (ctx.fundamentals.marketCapCr / 1000).toFixed(1) + 'K Cr' : ctx.fundamentals.marketCapCr.toFixed(0) + ' Cr') : 'N/A'}
- Analyst Target: ₹${ctx.fundamentals?.targetPrice || 'N/A'} (${ctx.fundamentals?.recommendation?.toUpperCase().replace(/_/g, ' ') || 'N/A'})
...
` : 'No specific stock context provided.';
  ...
  return `You are an expert financial analyst specialising in Indian equities listed on NSE/BSE. You have deep knowledge of technical analysis, fundamental analysis, and the nuances of the Indian stock market including sector dynamics, regulatory environment, and macroeconomic factors.
...
## Disclaimer
End each response with a single brief line: *For informational purposes only — not SEBI-registered investment advice.*`;
}
